...
# Primary rows use a square head; secondary markers and separators use a
# smaller square.  Named Powerline chrome remains visible in this script.
...
def normalize_diff_chrome(path: str, text: str) -> str:
    if path == DIFF_RENDER_SOURCE:
        old = 'let mut header_spans: Vec<RtSpan<\'static>> = vec!["• ".dim()];'
        new = 'let mut header_spans: Vec<RtSpan<\'static>> = vec!["■ ".dim()];'
        if text.count(old) == 1:
